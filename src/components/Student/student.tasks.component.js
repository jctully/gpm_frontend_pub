...
const Task = props => (
  <tr>
    ...
    <td className='checkbox'>
      <input type='checkbox' checked={props.task.task_completed} />
    </td>
    ...
    <td className={props.task.task_completed ? 'completed' : ''}>
      <a href={props.task.task_link}>{props.task.task_link}</a>{' '}
      {/* If link doesn't have http:// in link will redirect locally */}
    </td>
    <td>
...
export default class StudentTasks extends Component {
  ...
  componentDidMount() {
    axios
      .get(
        'https://gradprogmantest.herokuapp.com/tasks/student/' +
    ...
    axios
      .get(
        'https://gradprogmantest.herokuapp.com/students/' + this.props.studentId
      )
      .then(response => {
